...
const Login = () => {
  ...
  // Check if user is already logged in (OAuth callback)
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (session?.user) {
          ...
          // User is logged in, will be redirected by parent component
        }
      } catch (error) {
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setLoginError(error.message || 'Erreur de connexion avec GitHub');
      }
    } catch (error) {
